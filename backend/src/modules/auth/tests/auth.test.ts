...
describe('PATCH /api/v1/auth/me', () => {
  it('updates only the authenticated user profile and returns the new values', async () => {
    const registerRes = await registerAndGetTokens();
    const { accessToken } = registerRes.body.data;
    ...
    expect(updateRes.body.data.user).toMatchObject({
      firstName: 'Updated',
      lastName: 'Patient',
      phone: '+93700000000',
      preferredLanguage: 'da',
    ...
    expect(updateRes.body.data.user.password).toBeUndefined();
  });

  it('rejects protected identity fields', async () => {
    const registerRes = await registerAndGetTokens();
    const updateRes = await request(app)
      .patch('/api/v1/auth/me')
...
describe('password-reset email normalization', () => {
  it('finds a registered account when reset email uses different casing', async () => {
    await registerAndGetTokens();
    const token = await AuthService.initiatePasswordReset(validUser.email.toUpperCase());
...
describe('POST /api/v1/auth/reset-password', () => {
  it('resets the password with a valid one-time token and rejects the old password', async () => {
    const registration = await registerAndGetTokens();
    const oldAccessToken = registration.body.data.accessToken;
    const token = await AuthService.initiatePasswordReset(validUser.email);
    expect(token).toBeTruthy();
  ...
  it('rejects invalid or expired reset tokens', async () => {
    ...
    const res = await request(app)
      .post('/api/v1/auth/reset-password')
      .send({ token: 'invalid-reset-token', password: 'NewSecurePass2!' });
