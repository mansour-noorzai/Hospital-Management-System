...
export async function createLabOrder(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const order = await LabService.createLabOrder(parsed.data);
...
export async function listLabResults(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) return next(new AuthError());
    const parsed = ListLabResultsQuerySchema.safeParse(req.query);
    if (!parsed.success) return next(parseZodError(parsed.error));

    const result = await LabService.listLabResults(parsed.data, req.user._id, req.ownOnly);
    res.json(
      successResponse(result.data, {
        page: result.page,
        limit: result.limit,
