...
describe('Lab Routes', () => {
  ...
  it('17. GET /api/v1/lab/results — patient result list is restricted to own records', async () => {
    const ownOrder = await createLabOrder(adminToken, patientProfileId, doctorProfileId);
    const otherOrder = await createLabOrder(adminToken, secondPatientProfileId, doctorProfileId);

    for (const orderId of [ownOrder.body.data._id, otherOrder.body.data._id]) {
      const created = await request(app).post('/api/v1/lab/results').set('Authorization', `Bearer ${adminToken}`).send({
        labOrderId: orderId,
        results: [{ testCode: 'CBC-001', testName: 'CBC', value: '12.5', unit: 'g/dL', isNormal: true }],
      });
      expect(created.status).toBe(201);
    }

    const res = await request(app).get('/api/v1/lab/results?limit=100').set('Authorization', `Bearer ${patientToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(1);
    expect(res.body.data[0].patient._id?.toString?.() ?? res.body.data[0].patient.toString()).toBe(patientProfileId);
  });
