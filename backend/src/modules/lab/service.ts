...
import { emitToCurrentHospital } from '../../socket';
...
type ListLabResultsQuery = z.infer<typeof ListLabResultsQuerySchema>;
...
export async function createLabOrder(input: CreateLabOrderInput) {
  ...
  emitToCurrentHospital('lab.order.created', { orderId: order._id.toString() });
  return order;
}
...
export async function updateLabOrderStatus(orderId: string, input: UpdateOrderStatusInput) {
  ...
  await order.save();
  emitToCurrentHospital('lab.order.updated', { orderId: order._id.toString(), status: order.status });
  return order;
}
...
export async function createLabResult(input: CreateLabResultInput, technicianUserId: string) {
  ...
  emitToCurrentHospital('lab.result.updated', { resultId: result._id.toString(), orderId: result.labOrder.toString() });
  return result;
}

export async function listLabResults(
  filters: ListLabResultsQuery,
  requestingUserId: string,
  ownOnly?: boolean,
) {
  const page = filters.page ?? 1;
  ...
  const [data, total] = await Promise.all([
    LabResult.find(query)
      .populate({
        path: 'labOrder',
        populate: [
          { path: 'patient', populate: { path: 'userId', select: '-password' } },
          { path: 'doctor', populate: { path: 'userId', select: '-password' } },
        ],
      })
      .populate('technician', 'firstName lastName')
      .populate({ path: 'verifiedBy', populate: { path: 'userId', select: '-password' } })
