...
export const ListLabOrdersQuerySchema = z.object({
  ...
  status: z.string().refine(
    (value) => value.split(',').every(status => ['pending', 'in_progress', 'completed', 'cancelled'].includes(status.trim())),
    { message: 'Invalid lab order status value(s)' },
  ).optional(),
  priority: z.enum(['routine', 'urgent', 'stat']).optional(),
});
...
export const ListLabResultsQuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(100).optional(),
  status: z.string().refine(
    (value) => value.split(',').every(status => ['preliminary', 'final', 'amended'].includes(status.trim())),
    { message: 'Invalid lab result status value(s)' },
  ).optional(),
});
