...
export async function listDocuments(
  ...
  const [data, total] = await Promise.all([
    Document.find(query)
      ...
      .populate({ path: 'issuedBy', populate: { path: 'userId', select: '-password' } })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean(),
...
export async function getDocumentPdf(
  ...
  const authorizedDoc = await getDocumentById(documentId, requestingUserId, requestingRole, ownOnly);

  if (authorizedDoc.status !== 'issued') {
    throw new ValidationError('Only issued documents can be downloaded as PDF');
  }

  // Re-read without population so PDF fields use canonical ObjectIds rather than
  // populated subdocuments. Authorization has already been enforced above.
  const rawDoc = await Document.findById(documentId);
  if (!rawDoc) throw new NotFoundError('Document');

  const pdfBuffer = await generateDocumentPdf(rawDoc);
  return { buffer: pdfBuffer, filename: `${rawDoc.documentId}.pdf` };
}
