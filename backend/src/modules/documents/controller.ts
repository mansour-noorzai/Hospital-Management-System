...
export async function downloadDocumentPdf(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) return next(new AuthError());
    const { buffer, filename } = await DocumentService.getDocumentPdf(
      req.params.id,
      req.user._id,
      req.user.role,
      req.ownOnly
