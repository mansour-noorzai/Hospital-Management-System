...
export async function createInvoice(input: CreateInvoiceInput, issuedByUserId: string) {
  ...
  emitToCurrentHospital('invoice.updated', { invoiceId: invoice._id.toString(), status: invoice.status });
  return invoice;
}
...
export async function recordPayment(
  ...
  if (input.amount > invoice.balance + 0.000001) {
    throw new ValidationError(`Payment exceeds outstanding balance of ${invoice.balance}`);
  }
  ...
  await invoice.save();
  emitToCurrentHospital('payment.received', { invoiceId: invoice._id.toString(), status: invoice.status });
  return invoice;
}
