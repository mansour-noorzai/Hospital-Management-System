...
export async function listPatients(filters: { search?: string; page?: number; limit?: number; requestingUserId?: string; requestingRole?: string }) {
  const { search, page = 1, limit = 20 } = filters;
  const skip = (page - 1) * limit;

  const query: Record<string, unknown> = {};
  if (filters.requestingRole === 'doctor' && filters.requestingUserId) {
    const doctor = await Doctor.findOne({ userId: filters.requestingUserId }).lean();
    if (!doctor) return { data: [], total: 0, page, limit, totalPages: 0 };
    query._id = { $in: await Appointment.find({ doctor: doctor._id }).distinct('patient') };
  }
...
export async function getPatient(patientProfileId: string, requestingUserId: string, requestingRole: string) {
  ...
  if (requestingRole === 'doctor') {
    ...
    const authorized = doctor && await Appointment.exists({ doctor: doctor._id, patient: patient._id });
    if (!authorized) throw new ForbiddenError('You can only access patients assigned through your appointments');
  }
