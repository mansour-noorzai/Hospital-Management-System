...
beforeEach(async () => {
  await Hospital.create({ name: 'Test Hospital', systemName: 'Test HMS', shortName: 'TH', slug: 'test-hospital' });
});
...
describe('Patient Routes', () => {
  ...
  it('2. GET /api/v1/patients/:id — doctor cannot read an unassigned patient', async () => {
    // Create doctor
  ...
  it('7. GET /api/v1/patients — patient sees only their own profile', async () => {
    const first = await request(app).post('/api/v1/auth/register').send({
      firstName: 'Own', lastName: 'Patient', email: 'own.list@test.com', password: 'PatPass1!',
    });
    await request(app).post('/api/v1/auth/register').send({
      firstName: 'Other', lastName: 'Patient', email: 'other.list@test.com', password: 'PatPass1!',
    });
    const ownProfile = await Patient.findOne({ userId: first.body.data.user._id });

    const res = await request(app).get('/api/v1/patients?limit=100').set('Authorization', `Bearer ${first.body.data.accessToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(1);
    expect(res.body.data[0]._id).toBe(ownProfile!._id.toString());
  });

  it('8. POST /api/v1/patients — patient cannot create additional profiles', async () => {
    const reg = await request(app).post('/api/v1/auth/register').send({
      firstName: 'No', lastName: 'Duplicate', email: 'no.duplicate@test.com', password: 'PatPass1!',
    });
    const res = await request(app).post('/api/v1/patients').set('Authorization', `Bearer ${reg.body.data.accessToken}`).send({
      firstName: 'Extra', lastName: 'Profile', email: 'extra.profile@test.com', password: 'PatPass1!',
    });
    expect(res.status).toBe(403);
  });
