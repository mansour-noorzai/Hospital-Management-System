...
import { CreateAppointmentSchema, UpdateStatusSchema, GetSlotsSchema, ListAppointmentsQuerySchema } from './schema';
...
export async function listAppointments(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // Honour RBAC own-only access for both patients and doctors.
    if (req.ownOnly && req.user.role === 'patient') {
      ...
      if (!patientProfile) {
        res.json(successResponse([], { page: filters.page ?? 1, limit: filters.limit ?? 20, total: 0, totalPages: 0 }));
        return;
      }
      filters.patientId = patientProfile._id.toString();
    } else if (req.ownOnly && req.user.role === 'doctor') {
      const { Doctor } = await import('../../models/Doctor');
      const doctorProfile = await Doctor.findOne({ userId: req.user._id }).lean();
      if (!doctorProfile) {
        res.json(successResponse([], { page: filters.page ?? 1, limit: filters.limit ?? 20, total: 0, totalPages: 0 }));
        return;
      }
      filters.doctorId = doctorProfile._id.toString();
    }
