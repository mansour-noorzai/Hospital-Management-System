...
jest.mock('../../../socket', () => ({
  emitToUser: jest.fn(),
  emitToRole: jest.fn(),
  emitToCurrentHospital: jest.fn(),
  emitToHospital: jest.fn(),
  emitToHospitalRole: jest.fn(),
}));
...
beforeAll(async () => {
  mongoServer = await MongoMemoryServer.create({ instance: { args: ['--nounixsocket'] } });
...
describe('Appointment Routes', () => {
  ...
  it('8. GET /api/v1/appointments — patient list is restricted to own appointments', async () => {
    const otherPatUser = await User.create({
      firstName: 'List', lastName: 'Other', email: 'list.other@test.com', password: 'OtherPass1!', role: 'patient',
    });
    const otherPatProfile = await Patient.create({ userId: otherPatUser._id });

    await request(app).post('/api/v1/appointments').set('Authorization', `Bearer ${patientToken}`).send({
      doctorId: doctorProfileId, patientId: patientProfileId, date: FUTURE_DATE, timeSlot: '15:00', type: 'consultation',
    });
    await request(app).post('/api/v1/appointments').set('Authorization', `Bearer ${adminToken}`).send({
      doctorId: doctorProfileId, patientId: otherPatProfile._id.toString(), date: FUTURE_DATE, timeSlot: '15:30', type: 'consultation',
    });

    const res = await request(app).get('/api/v1/appointments?limit=100').set('Authorization', `Bearer ${patientToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data).toHaveLength(1);
    expect(res.body.data[0].patient._id.toString()).toBe(patientProfileId);
  });

  it('9. GET /api/v1/appointments/:id — patient cannot retrieve another patient appointment', async () => {
    const otherPatUser = await User.create({
      firstName: 'Direct', lastName: 'Other', email: 'direct.other@test.com', password: 'OtherPass1!', role: 'patient',
    });
    const otherPatProfile = await Patient.create({ userId: otherPatUser._id });
    const booked = await request(app).post('/api/v1/appointments').set('Authorization', `Bearer ${adminToken}`).send({
      doctorId: doctorProfileId, patientId: otherPatProfile._id.toString(), date: FUTURE_DATE, timeSlot: '16:00', type: 'consultation',
    });

    const res = await request(app).get(`/api/v1/appointments/${booked.body.data._id}`).set('Authorization', `Bearer ${patientToken}`);
    expect(res.status).toBe(403);
  });

  it('10. PATCH /api/v1/appointments/:id/status — rejects invalid status transitions', async () => {
    const booked = await request(app).post('/api/v1/appointments').set('Authorization', `Bearer ${patientToken}`).send({
      doctorId: doctorProfileId, patientId: patientProfileId, date: FUTURE_DATE, timeSlot: '16:30', type: 'consultation',
    });
    const res = await request(app)
      .patch(`/api/v1/appointments/${booked.body.data._id}/status`)
      .set('Authorization', `Bearer ${doctorToken}`)
