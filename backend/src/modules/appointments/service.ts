...
export async function bookAppointment(
  ...
  if (requestingRole === 'doctor' && doctor.userId.toString() !== createdByUserId) {
    throw new ForbiddenError('You can only book appointments in your own schedule');
  }
  ...
  const dayOfWeek = DAY_NAMES[appointmentDate.getUTCDay()];
  ...
  emitToUser(doctor.userId.toString(), 'appointment.created', { appointmentId: appointment._id.toString() });
  emitToUser(patient.userId.toString(), 'appointment.created', { appointmentId: appointment._id.toString() });
  return appointment;
}
...
export async function getAvailableSlots(doctorId: string, date: string) {
  ...
  const dayOfWeek = DAY_NAMES[new Date(date + 'T00:00:00.000Z').getUTCDay()];
  const availabilityForDay = doctor.availability.find(a => a.day === dayOfWeek);
...
export async function updateAppointmentStatus(
  ...
  if (!Types.ObjectId.isValid(appointmentId)) throw new ValidationError('Invalid appointment id');
  ...
  if (!allowedTransitions.includes(input.status)) {
    throw new ValidationError(`Cannot transition appointment from '${appointment.status}' to '${input.status}'`);
  }
  ...
  try {
    if (patientDoc.userId) {
      ...
      emitToUser(patientDoc.userId.toString(), 'appointment.updated', { appointmentId: appointment._id.toString(), status: input.status });
    }
    if (doctorDoc.userId) {
      ...
      emitToUser(doctorDoc.userId.toString(), 'appointment.updated', { appointmentId: appointment._id.toString(), status: input.status });
    }
  } catch {
...
export async function getAppointmentById(
  ...
  if (!Types.ObjectId.isValid(id)) throw new ValidationError('Invalid appointment id');
  ...
  if (ownOnly && requestingUserId) {
    if (requestingRole === 'patient') {
      const patient = appointment.patient as unknown as { userId?: { _id?: Types.ObjectId } | Types.ObjectId };
      const rawUserId = patient?.userId;
      const ownerId = rawUserId && typeof rawUserId === 'object' && '_id' in rawUserId
        ? rawUserId._id?.toString()
        : rawUserId?.toString();
      if (ownerId !== requestingUserId) throw new ForbiddenError('You can only view your own appointments');
    } else if (requestingRole === 'doctor') {
      const doctor = appointment.doctor as unknown as { userId?: { _id?: Types.ObjectId } | Types.ObjectId };
      const rawUserId = doctor?.userId;
      const ownerId = rawUserId && typeof rawUserId === 'object' && '_id' in rawUserId
        ? rawUserId._id?.toString()
        : rawUserId?.toString();
      if (ownerId !== requestingUserId) throw new ForbiddenError('You can only view your own appointments');
