...
export async function deleteDrug(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) return next(new AuthError());
    await PharmacyService.deleteDrug(req.params.id);
...
export async function createPrescription(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!parsed.success) return next(parseZodError(parsed.error));
    const prescription = await PharmacyService.createPrescription(parsed.data, req.user._id, req.user.role);
