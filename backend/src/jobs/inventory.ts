  ...
  cron.schedule('0 0 * * *', async () => {
    ...
    try {
      ...
      for (const item of expiringItems) {
        ...
        if (!item.hospitalId) continue;
        emitToHospitalRole(item.hospitalId.toString(), 'admin', 'inventory:expiring-soon', {
      ...
      for (const item of expiredItems) {
        if (!item.hospitalId) continue;
        emitToHospitalRole(item.hospitalId.toString(), 'admin', 'inventory:expired', {
          itemId: item._id.toString(),
          itemName: item.name,
