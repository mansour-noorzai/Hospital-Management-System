 ...
 *
 * The `authenticate` middleware must run first and set `req.user`.
...
/** Authentication context attached by the authenticate middleware. */
...
export function authorize(resource: Resource, action: Action) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const override = Types.ObjectId.isValid(user._id)
      ? await PermissionOverride.findOne({ userId: user._id }).lean()
      : null;
    const explicitlyDenied = override?.denies.includes(permissionKey) ?? false;
    const explicitlyGranted = override?.grants.includes(permissionKey) ?? false;

    if (explicitlyDenied || (!explicitlyGranted && !isActionAllowed(permission, action))) {
      res.status(403).json({
        success: false,
        error: {
          code: "FORBIDDEN",
