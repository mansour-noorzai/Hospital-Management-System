...
function labReport(order: LabOrder, result?: LabResult | null): PrintableReport {
  const patient = `${order.patient?.userId?.firstName ?? ''} ${order.patient?.userId?.lastName ?? ''}`.trim() || '—';
  const doctor = `${order.doctor?.userId?.firstName ?? ''} ${order.doctor?.userId?.lastName ?? ''}`.trim() || '—';
  return {
    ...
    fields: [
      ...
      { label: 'Ordered date', value: new Date(order.orderedAt ?? order.createdAt).toLocaleString() },
      { label: 'Priority', value: order.priority },
      { label: 'Order status', value: order.status },
      { label: 'Result status', value: result?.status },
      { label: 'Resulted at', value: result?.resultedAt ? new Date(result.resultedAt).toLocaleString() : '—' },
    ],
    tables: [
      {
        title: 'Ordered Tests',
        columns: ['Test', 'Code', 'Status'],
        rows: order.tests.map((test) => [test.name, test.code, test.status]),
      },
      ...(result ? [{
        title: 'Results',
        columns: ['Test', 'Value', 'Unit', 'Reference range', 'Normal'],
        rows: result.results.map((item) => [item.testName, item.value, item.unit, item.referenceRange, item.isNormal === undefined ? '—' : item.isNormal ? 'Yes' : 'No']),
      }] : []),
    ],
    notes: [order.notes, result?.notes].filter(Boolean).join('\n'),
  };
}
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-wrap gap-1">
          ...
          <PrintReportButton report={labReport(row)} />
          {canManageLab && (
            <>
              <Button
                variant="outline"
    ...
    <div className="space-y-6">
      ...
      <Dialog open={!!detailOrderId} onOpenChange={(open) => { if (!open) closeDetail(); }}>
        <DialogContent className="max-w-2xl" onClose={closeDetail}>
                          ...
                          {detailResult.results.map((r, idx) => (
                            <tr key={idx}>
                              ...
                              <td className="py-2 pr-4">
                                {r.value}
                                {r.unit && <span className="ms-1 text-muted-foreground">{r.unit}</span>}
                              </td>
          ...
          <DialogFooter>
            {detailOrder && <PrintReportButton report={labReport(detailOrder, detailResult)} />}
