...
function invoiceReport(invoice: Invoice, currency: string): PrintableReport {
  const money = (value: number) => `${value.toFixed(2)} ${currency}`;
  const patient = `${invoice.patient?.userId?.firstName ?? ''} ${invoice.patient?.userId?.lastName ?? ''}`.trim() || '—';
  return {
    ...
    fields: [
      ...
      { label: 'Issued date', value: invoice.issuedDate ? new Date(invoice.issuedDate).toLocaleDateString() : '—' },
      ...
      { label: `Tax (${invoice.taxRate}%)`, value: money(invoice.tax) },
      ...
      { label: 'Balance due', value: money(invoice.balance) },
      ...(invoice.insurance ? [
        { label: 'Insurance provider', value: invoice.insurance.provider },
        { label: 'Policy number', value: invoice.insurance.policyNumber },
        { label: 'Coverage', value: money(invoice.insurance.coverageAmount) },
      ] : []),
    ],
    tables: [
      {
        ...
        rows: invoice.lineItems.map((item) => [item.description, item.quantity, money(item.unitPrice), money(item.total)]),
      },
      ...(invoice.payments.length ? [{
        title: 'Payment History',
        columns: ['Date', 'Method', 'Reference', 'Amount'],
        rows: invoice.payments.map((payment) => [new Date(payment.paidAt).toLocaleString(), payment.method, payment.reference, money(payment.amount)]),
      }] : []),
    ],
    notes: invoice.notes,
  };
...
export function AdminBilling() {
  ...
  const { hospital } = usePreferences();
  const reportCurrency = hospital?.currency ?? 'AFN';
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-wrap gap-1">
          ...
          <PrintReportButton report={invoiceReport(row, reportCurrency)} />
    ...
    <div className="space-y-6">
      ...
      <Dialog open={!!detailInvoiceId} onOpenChange={(open) => { if (!open) closeDetail(); }}>
        <DialogContent className="max-w-2xl" onClose={closeDetail}>
          ...
          <DialogFooter>
            {detailInv && <PrintReportButton report={invoiceReport(detailInv, reportCurrency)} />}
