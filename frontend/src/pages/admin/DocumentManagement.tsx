...
function documentReport(document: Document): PrintableReport {
  const patient = `${document.patientId?.userId?.firstName ?? ''} ${document.patientId?.userId?.lastName ?? ''}`.trim() || '—';
  const doctor = `${document.issuedBy?.userId?.firstName ?? ''} ${document.issuedBy?.userId?.lastName ?? ''}`.trim() || '—';
  return {
    ...
    fields: [
      ...
      { label: 'Issued by', value: doctor },
      { label: 'Status', value: document.status },
      { label: 'Created', value: new Date(document.createdAt).toLocaleString() },
      { label: 'Issued at', value: document.issuedAt ? new Date(document.issuedAt).toLocaleString() : '—' },
      { label: 'Voided at', value: document.voidedAt ? new Date(document.voidedAt).toLocaleString() : '—' },
      ...Object.entries(document.templateData ?? {}).map(([label, value]) => ({ label: label.replace(/_/g, ' '), value })),
      { label: 'Content', value: document.content },
      { label: 'Void reason', value: document.voidReason },
    ],
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex gap-2">
          ...
          <PrintReportButton report={documentReport(row)} />
          {row.status === 'draft' && (
            <Button
              size="sm"
              ...
              <div className="space-y-5">
                ...
                {detailDocument.status === 'issued' && (
                  <div>
                    ...
                    <Button variant="outline" onClick={() => void downloadDocumentPdf(detailDocument._id, `${detailDocument.documentId}.pdf`)}>
                      Download PDF
                    </Button>
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  <PrintReportButton report={documentReport(detailDocument)} />
